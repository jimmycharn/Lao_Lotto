...
// 1. Add getMemberCommission helper before calculateTransferCommission
const getCommHelper = `
...
if (!content.includes('function getMemberCommission')) {
    content = content.replace('function calculateTransferCommission', getCommHelper + '\nfunction calculateTransferCommission');
}

// 2. Update userHistoriesWithProfiles mapping inside fetchHistoryDetails
...
// 4. Update header card stats to render hInComm and hInPay
content = content.replace(
