...
export function calculate4SetPrizes(betNumber, winningNumber, prizeSettings) {
...
export function checkBetWin(betType, betNumbers, winNums, payoutRate, amount, setPrice, setPrizes) {
    ...
    if (bt === 'run_top' && num.length === 1 && w3top) {
        if (w3top.includes(num))
            return { wins: true, payout: amount * payoutRate };
    }
    if (bt === 'run_bottom' && num.length === 1 && w2bottom) {
        if (w2bottom.includes(num))
            return { wins: true, payout: amount * payoutRate };
    }
...
function getCommissionRate(betType, userId, lotteryType, userSettingsMap) {
    const lotteryKey = lotteryType === 'lao' || lotteryType === 'hanoi' ? lotteryType : 'thai';
...
export function generateCandidateNumbers(betItems, lotteryType) {
    ...
    if (result.length > 5000) {
        result = result.filter(c => betItems.some(b => (b.bet_type === '4_set' && b.numbers === c) ||
    ...
    if (lotteryType === 'thai' && candidateData && typeof candidateData === 'object' && 'type' in candidateData && candidateData.type === 'thai') {
        ...
        for (const w3 of candidateData.top3) {
            ...
            for (const bet of betItems) {
                if (bet.net_amount <= 0)
                    continue;
                if (['2_bottom', 'run_bottom', 'pak_bottom', 'front_bottom_1', 'back_bottom_1'].includes(bet.bet_type))
                    continue;
                const result = checkBetWin(bet.bet_type, bet.numbers, winNums, bet.payout_rate, bet.net_amount, setPrice, bet.set_prizes);
...
export function greedyRecommendations(scenarios, betItems, budget, setPrice = 120, lotteryType = 'lao') {
    ...
    while (iterCount++ < maxIterations) {
        ...
        if (worstPayout <= payoutThreshold)
            break;
        ...
        if (transferAmount <= 0)
            transferAmount = is4set ? setPrice : 1;
        transferAmount = Math.min(transferAmount, currentRem);
        if (transferAmount <= 0)
            break;
