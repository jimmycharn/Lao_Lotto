...
function normalizeFourGroupShorthand(line: string): string {
    const clean = line.trim();
...
export function isAllSameDigits(numStr: string): boolean {
...
function normalizeUnicode(str: string): string {
    ...
    // Normalize float and position context shorthand variants (e.g. "ลอย บน" -> "ลอยบน", "วิ่ง บน" -> "วิ่งบน", "ลอย ล่าง" -> "ลอยล่าง")
    s = s.replace(/(?<![ก-๛a-zA-Z0-9])(วิ่ง|ลอย)[\s./+\-]?บน(?![ก-๛a-zA-Z0-9])/g, '$1บน');
    s = s.replace(/(?<![ก-๛a-zA-Z0-9])(วิ่ง|ลอย)[\s./+\-]?ล่าง(?![ก-๛a-zA-Z0-9])/g, '$1ล่าง');
    s = s.replace(/(?<![ก-๛a-zA-Z0-9])(วิ่ง|ลอย)[\s./+\-]?บ\.?(?![ก-๛a-zA-Z0-9])/g, '$1บน');
    s = s.replace(/(?<![ก-๛a-zA-Z0-9])(วิ่ง|ลอย)[\s./+\-]?ล\.?(?![ก-๛a-zA-Z0-9])/g, '$1ล่าง');
    s = s.replace(/(?<![ก-๛a-zA-Z0-9])(หน้า|หลัง|กลาง)[\s./+\-]?บน(?![ก-๛a-zA-Z0-9])/g, '$1บน');
    s = s.replace(/(?<![ก-๛a-zA-Z0-9])(หน้า|หลัง|กลาง)[\s./+\-]?ล่าง(?![ก-๛a-zA-Z0-9])/g, '$1ล่าง');
    s = s.replace(/(?<![ก-๛a-zA-Z0-9])ปัก[\s./+\-]?(หน้า|หลัง|กลาง)(?![ก-๛a-zA-Z0-9])/g, 'ปัก$1');
...
export function extractBuyerNote(text: string, lotteryType = 'lao'): string {
    ...
    const isLotteryTitleOnly = (str: string): boolean => {
    ...
    const isIgnoredHeaderLine = (line: string): boolean => {
        const trimmed = line.trim();
    ...
    const isIgnoredFooterLine = (line: string): boolean => {
        const trimmed = line.trim();
        if (!trimmed) return true;
        if (/^[\s\.\/\\\-_*#=~]+$/.test(trimmed)) return true;
        const cleanLower = trimmed.toLowerCase();
    ...
    const hasPurchaseNumbers = (line: string): boolean => {
        const trimmed = line.trim();
