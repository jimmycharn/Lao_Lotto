-- Migration: 153_fix_round_winner_missing_bet_types.sql
-- =====================================================
-- Fix: Support all missing/aliased 2-digit bet types (2_tang, 2_teng, 2_have, 2_back, 2_front_single) in winner calculation.
-- Also ensure they correctly map to their base payout/commission keys for settings lookup.
...
BEGIN
  ...
  LOOP
    ...
    -- ----- 2_RUN / 2_TENG / 2_HAVE (2 ตัวลอย / วิ่ง) -----
    ...
    IF v_is_winner AND v_submission.bet_type != '4_set' THEN
      -- Map position bet types to pak_top/pak_bottom/etc for settings lookup
      DECLARE
        v_settings_key TEXT;
      BEGIN
