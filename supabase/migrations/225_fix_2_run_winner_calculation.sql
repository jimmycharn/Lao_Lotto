-- Migration: 225_fix_2_run_winner_calculation.sql
-- Description:
-- 1. Fix calculate_round_winners for 2_run (2 ตัวลอย / วิ่ง 2 ตัวบน):
--    Change winning condition from v_2_top to v_3_top.
--    In 2_run, both picked digits must appear in 3_top.
-- 2. Fix fallback payout rate for 2_run to 10 (was incorrectly lumped with 2_top at 65).
-- 3. Reset is_winner = FALSE and prize_amount = 0 at the start of calculate_round_winners to ensure idempotency.
-- 4. Re-calculate winners for the current Thai round on 2026-09-16.
...
BEGIN
  ...
  LOOP
    ...
    -- ----- 2_RUN / 2_TENG / 2_HAVE (2 ตัวลอย / วิ่ง 2 ตัวบน) -----
    -- Both digits must appear in 3_top (v_3_top)
    IF (v_submission.bet_type = '2_run' OR v_submission.bet_type = '2_teng' OR v_submission.bet_type = '2_have') AND v_3_top IS NOT NULL AND length(v_3_top) = 3 AND length(v_num) = 2 THEN
...
-- Re-calculate round winners for active Thai round on 2026-09-16
SELECT calculate_round_winners('d506a9f2-c98c-434e-a603-b4a7697b86c2'::UUID);
SELECT calculate_round_winners('a58d4ec9-74e4-4525-acf1-282ba7d18f63'::UUID);
