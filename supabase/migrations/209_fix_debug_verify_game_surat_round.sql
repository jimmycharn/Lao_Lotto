-- Migration: 209_fix_debug_verify_game_surat_round.sql
...
BEGIN
    SELECT 
        COALESCE(SUM(CASE WHEN COALESCE(s.is_deleted, FALSE) = FALSE THEN s.amount ELSE 0 END), 0),
        ROUND(COALESCE(SUM(CASE WHEN COALESCE(s.is_deleted, FALSE) = FALSE THEN COALESCE(s.commission_amount, 0) ELSE 0 END), 0)),
        COALESCE(SUM(
            CASE 
