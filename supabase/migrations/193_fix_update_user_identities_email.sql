-- Migration: 193_fix_update_user_identities_email.sql
-- Description: Fix update_user_by_superadmin to not update generated email column on auth.identities
...
BEGIN
  ...
  IF new_email IS NOT NULL AND TRIM(new_email) != '' THEN
    ...
    IF v_clean_email != LOWER(COALESCE(v_current_email, '')) THEN
      ...
      IF EXISTS (SELECT 1 FROM auth.users WHERE LOWER(email) = v_clean_email AND id != target_user_id) THEN
      ...
      -- Update auth.identities (DO NOT update 'email' column directly as it is a generated column in Supabase Auth)
