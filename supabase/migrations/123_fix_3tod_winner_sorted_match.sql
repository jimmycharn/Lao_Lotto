-- =============================================
-- Fix: 3_tod (โต๊ด) winner calculation for pre-sorted numbers
-- =============================================
-- Problem: เต็งโต๊ด submissions store the number pre-sorted (e.g., 983 → 389).
-- The old logic required v_num != v_3_top to exclude exact matches (since 3_top 
-- handles those). But when the sorted number coincidentally equals the winning 
-- number (e.g., bet 983 sorted=389, winning=389), the tod check fails because
-- it thinks it's an exact match.
--
-- Fix: Remove the v_num != v_3_top condition for 3_tod and 3_tod_single.
-- The 3_top bet type already handles exact matches separately, so there's no
-- double-counting risk — a submission can only have ONE bet_type.
...
BEGIN
  ...
  LOOP
    ...
    -- ----- 3_TOD (3 ตัวโต๊ด) -----
    -- Win if sorted digits match (regardless of whether v_num equals v_3_top).
    -- Since submissions with bet_type='3_tod' store pre-sorted numbers,
    -- the old check (v_num != v_3_top) would incorrectly reject cases where
    -- the sorted number happens to equal the winning number.
    -- No double-counting risk: a submission has only ONE bet_type, so a 3_top
    -- entry and a 3_tod entry are always separate rows.
    ...
    IF v_is_winner AND v_submission.bet_type != '4_set' THEN
      -- Map position bet types to pak_top/pak_bottom for settings lookup
      DECLARE
        v_settings_key TEXT;
      BEGIN
      ...
      v_payout_rate := COALESCE(
        ...
        (SELECT payout_rate FROM type_limits WHERE round_id = p_round_id AND bet_type = v_settings_key LIMIT 1),
