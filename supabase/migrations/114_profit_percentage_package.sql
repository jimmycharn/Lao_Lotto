-- Migration: Add profit_percentage billing model
-- =====================================================
-- This adds a new billing model "profit_percentage" that:
-- 1. Uses percentage_rate for pending deduction during data entry (same as percentage)
-- 2. Uses profit_percentage_rate for final deduction after results announcement
-- 3. Allows negative credit balance (outstanding debt recovered on top-up)
-- =====================================================

-- 1. Add profit_percentage_rate column to subscription_packages
ALTER TABLE subscription_packages
ADD COLUMN IF NOT EXISTS profit_percentage_rate DECIMAL(5,2) DEFAULT 0;

COMMENT ON COLUMN subscription_packages.profit_percentage_rate IS 'เปอร์เซ็นต์คิดจากกำไร - ใช้กับ billing_model = profit_percentage';

-- 2. Update billing_model CHECK constraint to include profit_percentage
...
ALTER TABLE subscription_packages ADD CONSTRAINT subscription_packages_billing_model_check 
  CHECK (billing_model IN ('per_device', 'package', 'percentage', 'profit_percentage'));

-- Also update dealer_subscriptions billing_model constraint
...
-- 3. Add outstanding_debt column to dealer_credits for tracking negative balance recovery
ALTER TABLE dealer_credits
ADD COLUMN IF NOT EXISTS outstanding_debt DECIMAL(15,2) DEFAULT 0;

COMMENT ON COLUMN dealer_credits.outstanding_debt IS 'ยอดค้างตัดเครดิต - จะถูกหักอัตโนมัติเมื่อเติมเครดิต';

-- 4. Update finalize_round_credit to recognize profit_percentage (treat same as percentage during round closure)
...
BEGIN
    ...
    -- Check if dealer has percentage or profit_percentage billing
    IF v_round.billing_model IS NULL OR v_round.billing_model NOT IN ('percentage', 'profit_percentage') THEN
        RETURN jsonb_build_object(
            'success', TRUE,
...
-- 5. Update create_immediate_billing_record to recognize profit_percentage
...
-- 6. Allow dealers to insert their own credit_transactions
-- (needed by client-side deductProfitBasedCredit and updateDealerCredit)
DROP POLICY IF EXISTS "Dealers can insert own transactions" ON credit_transactions;
CREATE POLICY "Dealers can insert own transactions" ON credit_transactions
...
-- Done!
SELECT 'Migration 114_profit_percentage_package completed!' as status;
