-- Drop old 1-param overload if it still exists to prevent PGRST203 ambiguity
...
BEGIN
    ...
    IF (v_round.status = 'announced' OR v_round.is_result_announced = TRUE)
       AND NOT EXISTS (SELECT 1 FROM public.round_history WHERE round_id = p_round_id) THEN
        ...
        -- Insert dealer round_history including winning_numbers and Rule 1 round_date
        ...
        -- Insert per-user summary into user_round_history including winning_numbers and Rule 1 round_date
    ...
    -- NOTE: We intentionally PRESERVE member_round_payments and upstream_round_payments!
    -- These are financial transaction records that belong to the round's settlement history.
    -- Since the round is archived to round_history (keeping round_id = p_round_id),
    -- deleting payment records here would wipe out the dealer's payment history.
